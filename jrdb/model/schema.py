...
def translate(jp:str, endict:Dict[str,str] = {
    ...
    '順位':'rank','◎':'perfect','○':'great','▲':'soso','△':'bad','コード':'code','３Ｆ':'three_F',"オッズ":"odds","名":"name","印":"mark",
    "指数":"index","クラス":"class","コメント":"comment","距離":"distance","コース":"course","タイプ":"type","フラグ":"flag","データ":"data",
    "レース":"race","レースキー":"racekey","グレード":"grade","ペース":"pace","レースペース":"racepace","コーナー":"corner","ペースアップ":"paceup","レースコメント":"racecomment"
}) -> str:
    for o,n in prechar.items():
        jp = jp.replace(o,n)

    return "_".join(filter(lambda word:word!="",map(lambda item: endict[item['orig']] if item['orig'] in endict.keys() else item['hepburn'].strip(), kks.convert(jp))))